...
describe('Extract the test descriptions from a block of source code (mostly a file)', () => {
  describe('GIVEN a string', () => {
    it('WHEN it is empty THEN return no test suites', () => {
      assert.deepStrictEqual(extractTestSuites(''), {name: '', suites: [], tests: [], origin: ''});
    });
    it('WHEN it contains not test THEN return no test suites', () => {
      assert.deepStrictEqual(extractTestSuites('var x = 1; // but no test'), {name: '', suites: [], tests: [], origin: ''});
    });
